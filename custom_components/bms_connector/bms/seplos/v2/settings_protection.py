...
def decode_fourseven(hex_string):
    ...
    datai_values = [
        ...
        # THERE ARE ERRORS BEYOND THIS POINT
        (int.from_bytes(datai_bytes[84:86], byteorder='big')  - 2731) / 10.0,  # Environmental over-temperature protection
        (int.from_bytes(datai_bytes[86:88], byteorder='big')  - 2731) / 10.0,  # Environmental overtemperature recovery
        (int.from_bytes(datai_bytes[88:90], byteorder='big')  - 2731) / 10.0,  # Environmental under-temperature protection
        (int.from_bytes(datai_bytes[90:92], byteorder='big')  - 2731) / 10.0,  # Environmental undertemperature recovery
        (int.from_bytes(datai_bytes[92:94], byteorder='big')  - 2731) / 10.0,  # Power high temperature alarm: 373.15 K
        (int.from_bytes(datai_bytes[94:96], byteorder='big')  - 2731) / 10.0,  # Power high temperature recovery: 373.15 K
        (int.from_bytes(datai_bytes[96:98], byteorder='big')  - 2731) / 10.0,  # Power over temperature protection: 358.15 K
        (int.from_bytes(datai_bytes[98:100], byteorder='big')  - 2731) / 10.0,  # Power over temperature recovery: 273.15 K
        int.from_bytes(datai_bytes[100:102], byteorder='big') / 100.0,  # Charging overcurrent warning: 200.00 A
        int.from_bytes(datai_bytes[102:104], byteorder='big') / 100.0,  # Charging overcurrent recovery: 195.00 A
        int.from_bytes(datai_bytes[104:106], byteorder='big') / 100.0,  # Discharge overcurrent warning: -205.00 A
        int.from_bytes(datai_bytes[106:108], byteorder='big') / 100.0,  # Discharge overcurrent recovery: -203.00 A
        int.from_bytes(datai_bytes[108:110], byteorder='big') / 100.0,  # Charge overcurrent protection: 210.00 A
        int.from_bytes(datai_bytes[110:112], byteorder='big') / 100.0,  # Discharge overcurrent protection: -210.00 A
        int.from_bytes(datai_bytes[112:114], byteorder='big') / 100.0,  # Transient overcurrent protection: -300.00 A
        int.from_bytes(datai_bytes[114:116], byteorder='big') / 1000.0,  # Output soft start delay: 2000 ms
        int.from_bytes(datai_bytes[116:118], byteorder='big') / 100.0,  # Battery rated capacity: 230.00 Ah
        int.from_bytes(datai_bytes[118:120], byteorder='big') / 100.0,  # SOC: 100.00 Ah
        int.from_bytes(datai_bytes[120:122], byteorder='big') / 100.0,  # Cell invalidation differential pressure: 0.50 V
        int.from_bytes(datai_bytes[122:124], byteorder='big') / 100.0,  # Cell invalidation recovery: 0.30 V
        int.from_bytes(datai_bytes[124:126], byteorder='big') / 1000.0,  # Equalization opening pressure difference: 0.030 V
        int.from_bytes(datai_bytes[126:128], byteorder='big') / 1000.0,  # Equalization closing pressure difference: 0.020 V
        int.from_bytes(datai_bytes[128:130], byteorder='big'),  # Static equilibrium time: 10 sec
        int.from_bytes(datai_bytes[130:132], byteorder='big'),  # Battery number in series: 16 String
        int.from_bytes(datai_bytes[132:134], byteorder='big') / 1000.0,  # Charge overcurrent delay: 10 sec
        int.from_bytes(datai_bytes[134:136], byteorder='big') / 1000.0,  # Discharge overcurrent delay: 10 sec
        int.from_bytes(datai_bytes[136:138], byteorder='big') / 1000.0,  # Transient overcurrent delay: 30 ms
        int.from_bytes(datai_bytes[138:140], byteorder='big') / 1000.0,  # Overcurrent delay recovery: 60 sec
        int.from_bytes(datai_bytes[140:142], byteorder='big'),  # Overcurrent recovery times: 5 times
        int.from_bytes(datai_bytes[142:144], byteorder='big') / 60.0,  # Charge current limit delay: 5 min
        int.from_bytes(datai_bytes[144:146], byteorder='big') / 60.0,  # Charge activation delay: 1 min
        int.from_bytes(datai_bytes[146:148], byteorder='big'),  # Charging activation interval: 10 sec
        int.from_bytes(datai_bytes[148:150], byteorder='big'),  # Charge activation times: 10 times
        int.from_bytes(datai_bytes[150:152], byteorder='big') / 60.0,  # Work record interval: 30 min
        int.from_bytes(datai_bytes[152:154], byteorder='big') / 60.0,  # Standby recording interval: 240 min
        int.from_bytes(datai_bytes[154:156], byteorder='big'),  # Standby shutdown delay: 48 sec
        int.from_bytes(datai_bytes[156:158], byteorder='big') / 100.0,  # Remaining capacity alarm: 15 %
        int.from_bytes(datai_bytes[158:160], byteorder='big') / 100.0,  # Remaining capacity protection: 5 %
        int.from_bytes(datai_bytes[160:162], byteorder='big') / 100.0,  # Interval charge capacity: 96 %
        int.from_bytes(datai_bytes[162:164], byteorder='big') / 100.0,  # Cycle cumulative capacity: 80 %
        int.from_bytes(datai_bytes[164:166], byteorder='big') / 1000.0,  # Connection fault impedance: 10.0 mΩ
        int.from_bytes(datai_bytes[166:168], byteorder='big'),  # Compensation point 1 position: 9 String
        int.from_bytes(datai_bytes[168:170], byteorder='big') / 1000.0,  # Compensation point 1 impedance: 0.0 mΩ
        int.from_bytes(datai_bytes[170:172], byteorder='big'),  # Compensation point 2 position: 13 String
        int.from_bytes(datai_bytes[172:174], byteorder='big') / 1000.0,  # Compensation point 2 impedance: 0.0 mΩ
        int.from_bytes(datai_bytes[174:176], byteorder='big'),  # Compensation point 3 position: 0 String
        int.from_bytes(datai_bytes[176:178], byteorder='big') / 1000.0,  # Compensation point 3 impedance: 0.0 mΩ
        int.from_bytes(datai_bytes[178:180], byteorder='big'),  # Compensation point 4 position: 0 String
        int.from_bytes(datai_bytes[180:182], byteorder='big') / 1000.0  # Compensation point 4 impedance: 0.0 mΩ
    ]
    ...
    result.charge_activation_interval = datai_values[73]
    ...
    result.compensation_point_3_position = datai_values[87]
    result.compensation_point_3_impedance = datai_values[88]
    result.compensation_point_4_position = datai_values[89]
    result.compensation_point_4_impedance = datai_values[90]
